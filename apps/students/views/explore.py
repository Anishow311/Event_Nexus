...
def explore(request):
    ...
    if query.strip():
        # Filter ClubProfile: ONLY return clubs where is_approved=True
        ...
        clubs = ClubProfile.objects.filter(is_approved=True).filter(club_q)

        # Filter Event: ONLY pull events from is_approved=True clubs
        ...
        events = Event.objects.filter(club__is_approved=True).filter(event_q)
